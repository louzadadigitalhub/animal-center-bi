...
function unitSlice(meta, year, month) {
  ...
  return {
    ...
    recebido,
    caixa: {
      noDia: Math.round(recebido * 0.39),
      posteriores: Math.round(recebido * 0.61),
      adiantamento: 0,
      receitaTotal: recebido,
      emAberto: Math.round(fatVenda - recebido),
    },
    consultas,
    vacinasAplicadas,
...
export function getView(unitId, year, month) {
  ...
  return {
    ...
    caixa: {
      noDia: (a.caixa?.noDia || 0) + (b.caixa?.noDia || 0),
      posteriores: (a.caixa?.posteriores || 0) + (b.caixa?.posteriores || 0),
      adiantamento: (a.caixa?.adiantamento || 0) + (b.caixa?.adiantamento || 0),
      receitaTotal: (a.caixa?.receitaTotal || a.recebido) + (b.caixa?.receitaTotal || b.recebido),
      emAberto: (a.caixa?.emAberto || 0) + (b.caixa?.emAberto || 0),
    },
