...
export async function planilhaDre(matriz) {
  ...
  for (const parte of matriz.partes) {
    ...
    ws.mergeCells("A2:N2");
    ws.getCell("A2").value =
      `Demonstrativo do SimplesVet, regime de ${matriz.regime}. Valores em R$. Extraido em ${dataBr(matriz.at)}.`;
    ws.getCell("A2").font = { size: 10, color: { argb: "FF666666" } };

    ws.mergeCells("A3:N3");
    ws.getCell("A3").value =
      matriz.ultimoRealizado >= 0 && matriz.ultimoRealizado < 11
        ? `Realizado ate ${MESES[matriz.ultimoRealizado]}. As colunas seguintes tem despesa ja lancada (aluguel, salario, parcelas a vencer) e receita ainda nao vendida — nao leia aquele saldo como prejuizo previsto. O acumulado soma so ate ${MESES[matriz.ultimoRealizado]}.`
